    ...
    // ── Upload ──
        ...
        if ($nb_photos + $nb_nouvelles > 6) {
            ...
            for ($i = 0; $i < $nb_nouvelles; $i++) {
                ...
                if (!$photo_principale) {
                    $stmt = $pdo->prepare("UPDATE users SET photo = ? WHERE id = ?");
                    $stmt->execute([$result['nom'], $user_id]);
                    $photo_principale = $result['nom'];
                }
    ...
    <?php foreach ($photos as $photo): ?>
      <div style="position: relative;">
        ...
        <?php if ($photo_principale === $photo['nom_fichier']): ?>
          <span style="position: absolute; left: 8px; top: 8px; background: #eef8f1; color: #216b36; border: 0.5px solid #9ed3ad; border-radius: 999px; padding: 4px 10px; font-size: 11px;">
            Principale
          </span>
        <?php endif; ?>
