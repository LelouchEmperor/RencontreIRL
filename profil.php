...
if (!$user) {
    header('Location: /Site_rencontre/RencontreIRL/auth/deconnexion.php');
    exit;
}
    ...
    <div class="profil-sidebar">
      <div onclick="document.getElementById('photo').click()" style="cursor: pointer; position: relative; display: inline-block; margin-bottom: 1rem;">
        <?php if (!empty($user['photo'])): ?>
          <img
            src="/Site_rencontre/RencontreIRL/uploads/<?= htmlspecialchars($user['photo']) ?>"
            id="sidebarPreview"
            alt="Photo de profil"
            ...
            <?php foreach ($mes_sorties as $sortie): ?>
              <div class="profil-sortie-card">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                  ...
                  <span style="font-size: 12px; color: #a07080;"><?= (int) $sortie['nb_participants'] ?> participant(s)</span>
                </div>
                ...
                <div style="display: flex; gap: 0.75rem; margin-top: 0.75rem;">
                  <a href="modifier-sortie.php?id=<?= (int) $sortie['id'] ?>" class="cta-btn-small">Modifier</a>
                  <a href="supprimer-sortie.php?id=<?= (int) $sortie['id'] ?>" style="font-size: 12px; color: #8b1a2a; border: 0.5px solid #d4909a; padding: 6px 14px; border-radius: 6px; text-decoration: none;">Supprimer</a>
                </div>
              </div>
            <?php endforeach; ?>
          </div>
            ...
            <?php foreach ($sorties_rejointes as $sortie): ?>
              <div class="profil-sortie-card">
                ...
                <div style="margin-top: 0.75rem;">
                  <a href="quitter-sortie.php?id=<?= (int) $sortie['id'] ?>" style="font-size: 12px; color: #8b1a2a; border: 0.5px solid #d4909a; padding: 6px 14px; border-radius: 6px; text-decoration: none;">Quitter la sortie</a>
                </div>
              </div>
            <?php endforeach; ?>
          </div>
